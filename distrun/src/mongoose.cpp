...
#pragma warning (disable : 4996)  // missing c99 support
#endif
...
static void mg_http_req_handler(struct ns_connection *nc, int ev, void *p);
...
static void forward_put_data(struct connection *conn) {
  ...
  size_t k = conn->cl < (int64_t) io->len ? (size_t)conn->cl : (size_t) io->len;   // To write
...
struct mg_connection *mg_connect(struct mg_server *server, const char *addr) {
  ...
  nsconn = ns_connect(&server->ns_mgr, addr, mg_http_req_handler, NULL);
    ...
    case NS_POLL:
      ...
      // Expire idle connections
      {
        ...
        if (nc->listener != NULL &&
          ...
          mg_http_req_handler(nc, NS_CLOSE, NULL);
          nc->flags |= NSF_CLOSE_IMMEDIATELY;
        }
      }
      break;
...
////////////////////////////////////////////////////////////////////////////////
string peer_ip(struct ns_connection *nc) {
  return string(inet_ntoa(((struct sockaddr_in*)(&nc->sa.sa))->sin_addr));
}

////////////////////////////////////////////////////////////////////////////////
struct Msg {
  Msg() {/**/}
  Msg(const string& cmd) { content_ = cmd; }

  Msg& attr(const string& k, const string& v) {
    content_ += "|" + k + ":" + v + "|";
    return *this;
  }

  string to_string()     const { return content_; }
  operator string()      const { return content_; }
  const char* c_str()    const { return content_.c_str(); }
  
  const char* operator[](const string& k) {
    string key_part = "|" + k + ":";

    size_t key_start = content_.find(key_part);
    if (key_start == string::npos)
      return "";

    size_t value_start = key_start + key_part.size();
    size_t value_end = content_.find("|", value_start);
    if ( value_end == string::npos)
      return "";

    string value = content_.substr(value_start, value_end - value_start);
    std::pair<set<string>::iterator, bool> ret = values_.insert(value);
...
static const char *s_no_cache_header = "Cache-Control: max-age=0, post-check=0, "
  "pre-check=0, no-store, no-cache, must-revalidate\r\n";

void udp_broadcast(const string& msg, int port) {
  printf("--- broadcast: %s.\n", msg.c_str());
  ...
  int ret = sendto( s
                  ...
                  , msg.size()
                  , 0
                  , (sockaddr*)&brdcastaddr, sizeof(brdcastaddr));
  if(ret == (int)msg.size())
    cout << "--- Broadcasting is done: " << msg << endl;
  
  ::closesocket(s);
}

#include "dist_runner.hpp"
